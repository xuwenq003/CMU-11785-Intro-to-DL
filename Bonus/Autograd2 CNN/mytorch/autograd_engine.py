...
#        print(backward_operation)
        for input in inputs:
#            print(input)
            self.memory_buffer.add_spot(input)
...
#            print(i)
            if i==0:
                grad_z = np.ones_like(operation.output)*divergence
            else:
                grad_z = self.memory_buffer.get_param(operation.output).reshape(operation.output.shape)
            grad = operation.backward_operation(grad_z, *operation.inputs)
#            print(grad)
            for i, input in enumerate(operation.inputs):
#                print(operation.inputs)
                # skip connection
                if self.memory_buffer.is_in_memory(input):
                    ori_grad = self.memory_buffer.get_param(input)
                    self.memory_buffer.set_param(input, grad[i]+ori_grad)
                else:
                    self.memory_buffer.set_param(input, grad[i])
            for i, grad_2_up in enumerate(operation.gradients_to_update):
                if grad_2_up is not None:
                    grad_2_up+=grad[i]
            i+=1
