...
class MLP(object):
    ...
    def __init__(self, input_size, output_size, hiddens, activations, weight_init_fn,
        ...
        self.linear_layers = [Linear(input, output, weight_init_fn, bias_init_fn) for (input, output) in zip(layers[:-1],layers[1:])]
        
        # If batch norm, add batch norm layers into the list 'self.bn_layers'
        if self.bn:
            self.bn_layers = [BatchNorm(input) for input in layers[1: num_bn_layers+1]]
        self.num_bn_layers=num_bn_layers
    ...
    def forward(self, x):
        ...
        for layer in range(self.nlayers):
            ylin=self.linear_layers[layer].forward(y)
            if self.bn and layer<self.num_bn_layers:
                if self.train_mode:
                    ylin=self.bn_layers[layer].forward(ylin, eval=False)
                else:
                    ylin=self.bn_layers[layer].forward(ylin, eval=True)
            y=self.activations[layer].forward(ylin)
        self.y=y
        return y
    ...
    def backward(self, labels):
        ...
        celoss=self.criterion.forward(self.y, labels)
        grady = self.criterion.derivative()
        for layer in reversed(range(0, self.nlayers)):
            gradylin=self.activations[layer].derivative()*grady
            if self.bn and layer<self.num_bn_layers:
                gradylin=self.bn_layers[layer].backward(gradylin)
            grady=self.linear_layers[layer].backward(gradylin)
        return grady
